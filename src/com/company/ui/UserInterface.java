...
public class UserInterface {
    ...
    private void processSale(){
        while (true){
            askToCreateNewSale();
            Introduction();
            askCustomerIDforCustomerDiscount();
            askToEnterItems();
            endSale();
    ...
    private void askCustomerIDforCustomerDiscount() {
        System.out.println("Please enter costumer ID for discount");
        String  answer = input.nextLine();
        Customer customer = register.getCustomer(answer);
        if(customer != null ){
            register.enterCustomerForDiscount(customer);
        }
        else {
    ...
    private void Introduction(){
        String s ="Product2 has %10 discount \n";
        s += "Product5 has %20 discount\n";
